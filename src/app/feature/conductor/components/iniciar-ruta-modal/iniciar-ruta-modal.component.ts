...
@Component({
  selector: 'app-iniciar-ruta-modal',
  standalone: true,
  imports: [DecimalPipe, NgClass, ReactiveFormsModule, MapPointPickerComponent],
  templateUrl: './iniciar-ruta-modal.component.html',
})
export class IniciarRutaModalComponent implements OnInit {
  ...
  protected submit(): void {
    if (this.form.invalid) {
      ...
      toast.warning('Revisa el odómetro y la ubicación en el mapa.');
      return;
    }
    ...
    if (odometro == null) {
      ...
      toast.warning('Ingresa el odómetro del vehículo.');
      return;
    }

    this.loadingService.setLoading(true);
    this.ejecucionRutasService
      .iniciar(this.ejecucionRutaId(), {
        latitudInicio: raw.latitud,
        longitudInicio: raw.longitud,
        odometroInicio: odometro,
      })
      .pipe(
        catchError(() => {
          toast.error('No se pudo iniciar la ruta. Inténtalo de nuevo.');
          return EMPTY;
        }),
